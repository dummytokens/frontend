...
export type NetworkType = 'sepolia'|'arbitrum'|'linea'|'optimism'|'polygon'|'avalanche'|'bsc'
...
export default function Network({network}: Props) {
    ...
    return <div onClick={()=>select(network)} className={`
        active:scale-95 rounded-full align-middle p-1
        border-4 ${isSelected?'border-slate-500':'border-slate-500/10 hover:border-slate-500'} 
        cursor-pointer transition-all w-8 h-8 
    `}>
        <Image className='m-auto align-middle w-full h-full' src={ICON[network]} alt={network}/>
    </div>
}